...
def get(url)->requests.Response:
    ...
    while True:
        try:
            ...
            fail += 1
            if fail > 10:
                subprocess.run(["notify-send", f"Failed to get weather data\n{e}"])
                sys.exit(0)
            time.sleep(1)


def get_city_by_ip():
    ip = get("https://ipinfo.io").json()["ip"]
    return get("https://ipinfo.io/" + ip + "/json").json()["city"]

# city = get_city_by_ip()
city = "Shanghai"

weather = get("https://wttr.in/" + city + "?format=j1").json()
